...
def get_datasets(json_filepath):
    ...
    for _, row in df.iterrows():
        ...
        for _ in range(5):
            ...
            training_labels.append(row['_id'])  # The label for the distorted image is the same as the original image
        ...
        for _ in range(2):
            ...
            testing_labels.append(row['_id'])  # The label for the distorted image is the same as the original image
    ...
    training_labels = le.fit_transform(training_labels) # Convert the labels into numpy arrays and encode them as integers
...
def create_smaller_json(filepath, image_size=-1, rand=False):
    ...
    if rand and image_size==-1:
        ...
        print(f'Copying {image_size} RANDOM (unique) objects from \"{filepath}\" to \"{new_filepath}\" ...\n')
...
def filter_attributes_json(filepath, attributes=['_id', 'image_uris', 'card_faces']):
    ...
    filtered_data = [{attr: obj[attr] for attr in attributes if attr in obj} for obj in data]
...
def format_image_attributes(filepath, image_size='normal'):
    ...
    for item in data:
        if 'image_uris' in item:  # Replace 'attribute_to_delete' with the name of the attribute you want to delete
            image_url = item['image_uris'][image_size]
            del item['image_uris']
            if 'card_faces' in item: del item['card_faces']
