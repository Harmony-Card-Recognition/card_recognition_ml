...
import json
from helper.json_processing import format_json, encode_alphanumeric_to_int 
...
def enqueue(raw_json_filepath:str, inital_json_grab: int, model_filepath:str, image_size:str) -> str:
    new_filepath = os.path.join(model_filepath, "unprocessed.json") 
    format_json(raw_json_filepath=raw_json_filepath, new_filepath=new_filepath, image_count=inital_json_grab, image_size=image_size)
    encode_alphanumeric_to_int(new_filepath) 
    return new_filepath
